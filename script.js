...
function row_insert(dataset_counts) {
    ...
    let row_heading_lenght=row_headings.length
        ...
        if (`$dataset-${dataset_counts}`==`$dataset-${dataset_counts}`){
            
        input_tag.setAttribute("id",`rows-${dataset_counts}-${row_headings[j]}-${add_row}`)
...
add_row++  
count_array.push(add_row)
count_array.push(sum(count_array))
...
function printData(){
    ...
    for(let i=0;i<dataset_count;i++){
        ot.innerHTML +=`- name: ${document.getElementById(`name-${i}`).value} <br>`;
        ot.innerHTML +=`load: ${document.getElementById(`load_type-${i}`).value} <br>`;
        for(let j=0;j<add_row;j++){
            console.log(`rows-${i}-column_name-${j}`)
